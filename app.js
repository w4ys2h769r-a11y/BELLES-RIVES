...
function correctKnownData(normalized) {
  ...
  // On ne touche qu'aux installations qui ont encore la valeur par défaut d'origine, pour
  // ne jamais écraser une modification volontaire faite depuis l'application.
  normalized.members = (normalized.members || []).map((member) => {
    if (member.name !== "Pierrick" || member.building !== "Batiment E") return member;
    return {
      ...member,
      building: "Batiment C",
