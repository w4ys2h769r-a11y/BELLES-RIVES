...
const PROFILES = ["Conseil syndical", "Céline", "Edna", "Cécile", "Beïja", "Christina", "Justine", "Pierrick"];
...
const seedData = {
  members: [
    ...
    },
    {
      id: crypto.randomUUID(),
      name: "Pierrick",
      role: "Référent bâtiment E",
      building: "Batiment E",
      email: "",
      phone: "",
      notes: "Membre du conseil syndical pour le bâtiment E."
    }
  ],
  meetings: [
    {
...
const schemas = {
  ...
  contracts: [
    ...
    ["noticeMonths", "Préavis", "select", "", ["1", "2", "3", "4", "5", "6"]],
  ...
  members: [
    ["name", "Nom", "text"],
    ["role", "Rôle", "text"],
    ["building", "Référent", "select", "", BUILDINGS],
    ["email", "Email", "email"],
    ["phone", "Téléphone", "tel"],
    ["notes", "Notes", "textarea", "full"]
  ]
...
function normalizeTabs(tabs = []) {
  ...
  return DEFAULT_TABS.map((base, index) => ({
    ...base,
    ...(byId.get(base.id) || {}),
    icon: base.icon,
...
function normalizeState(data) {
  ...
  ["members", "meetings", "tasks", "incidents", "syndicRequests", "budget", "invoices", "documents", "contracts", "procedures", "contacts"].forEach((collection) => {
...
function setView(view, updateHistory = true) {
  ...
  document.getElementById("quickAdd").style.display = ["dashboard", "urgencies", "documents", "accounting", "settings"].includes(view) ? "none" : "inline-flex";
  render();
}
...
function renderMembers() {
  const members = filtered("members");
  document.getElementById("memberCards").innerHTML = members.length
    ? members
        .map((member) => {
      ...
      <article class="card ${buildingClass(member.building)}">
        ...
        .join("")
    : empty("Aucun membre du conseil syndical.");
}
...
async function saveForm(event) {
  ...
  if ("amount" in data) data.amount = Number(data.amount);
...
document.getElementById("resetTabs").addEventListener("click", () => {
  if (confirm("Rétablir les onglets par défaut ?")) {
